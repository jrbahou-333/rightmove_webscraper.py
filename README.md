...
GitHub only fires `schedule` triggers from the **default branch (`master`)**, and the
workflow checks out `master` normally — so anything you want to go live (e.g. a new
search in `src/searches.py`) just needs to be merged/pushed to `master`.
